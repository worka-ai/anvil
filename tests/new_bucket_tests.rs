...
async fn test_delete_bucket_soft_deletes_and_enqueues_task() {
    common::with_test_dbs(|global_db_url, regional_db_url, _|
        async move {
            ...
            // 2. Delete the bucket
            let mut del_req = Request::new(DeleteBucketRequest { bucket_name: bucket_name.clone() });
            ...
            let list_res_after_delete = bucket_client.list_buckets(list_req_after_delete).await.unwrap().into_inner();
