...
async fn test_auth_flow_with_wildcard_scopes() {
    common::with_test_dbs(|global_db_url, regional_db_url, _| async move {
        ...
        assert!(
            common::wait_for_port(grpc_addr, std::time::Duration::from_secs(5)).await,
        ...
        let mut auth_client = AuthServiceClient::connect("http://127.0.0.1:50099")
            .await
            .unwrap();
        ...
        let mut bucket_client = BucketServiceClient::connect("http://127.0.0.1:50099")
            .await
            .unwrap();
