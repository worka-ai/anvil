...
async fn test_grant_and_revoke_access() {
    ...
    let granter_token = common::get_auth_token_for_app(&world.global_db_url, &world.grpc_addr, "granter-app", "*", "*").await;
    let (grantee_client_id, grantee_client_secret) = common::create_app(&world.global_db_url, "grantee-app");
    ...
    let grantee_token = common::get_token_for_scopes(&world.grpc_addr, &grantee_client_id, &grantee_client_secret, vec![format!("read:{}", resource)]).await;
    ...
    let res = common::try_get_token_for_scopes(&world.grpc_addr, &grantee_client_id, &grantee_client_secret, vec![format!("read:{}", resource)]).await;
    assert!(res.is_err());
}
...
async fn test_set_public_access_and_get() {
    let world = TestWorld::new().await;
    let mut auth_client = AuthServiceClient::connect(world.grpc_addr.clone()).await.unwrap();
    let mut object_client = ObjectServiceClient::connect(world.grpc_addr.clone()).await.unwrap();
