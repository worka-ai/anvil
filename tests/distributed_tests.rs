...
async fn test_distributed_reconstruction_on_node_failure() {
    common::with_test_dbs(|global_db_url, regional_db_url, _|
        async move {
            ...
            let mut chunks = vec![PutObjectRequest { data: Some(anvil::anvil_api::put_object_request::Data::Metadata(metadata)) }];
            ...
            assert_eq!(downloaded_data, content, "Reconstructed data did not match original data");
        }
    ).await;
}
