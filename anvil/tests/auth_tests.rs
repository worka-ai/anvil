...
use anvil::anvil_api::{
    ...
    CreateHostAliasRequest, CreateObjectLinkRequest, DeleteApplicationCredentialRequest,
...
async fn test_grant_and_revoke_access() {
    ...
    // Public delegation is non-escalating: the granter must already hold the
    // capability it delegates, in addition to policy management authority.
    grant_policy(&cluster, "granter-app", "bucket:read", &resource).await;
...
async fn tenant_can_create_update_delete_object_link() {
    ...
    let limited_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "limited-link-reader".to_string(),
            vec![
                format!("object:list|{bucket_name}/releases/"),
                format!("object:read|{bucket_name}/{link_key}"),
            ],
            1,
        )
        .unwrap();
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "tenant-b-link-reader".to_string(),
            vec![format!("object:read|{bucket_name}/{link_key}")],
            2,
        )
        .unwrap();
...
async fn tenant_can_request_and_verify_host_alias() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "host-alias-tenant-b".to_string(),
            vec![
                format!("bucket:read|{bucket_name}"),
                format!("bucket:write|{bucket_name}"),
            ],
            2,
        )
        .unwrap();
...
async fn tenant_object_link_cannot_cross_tenant_without_delegation() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "tenant-b-link-writer".to_string(),
            vec![
                format!("object:write|{bucket_name}/latest"),
                format!("object:read|{bucket_name}/target"),
            ],
            2,
        )
        .unwrap();
...
async fn tenant_link_list_is_authz_filtered() {
    ...
    let limited_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "limited-link-lister".to_string(),
            vec![
                format!("object:list|{bucket_name}/releases/"),
                format!("object:read|{bucket_name}/releases/visible"),
            ],
            1,
        )
        .unwrap();
...
async fn tenant_can_create_and_rotate_own_app_secret() {
    ...
    let mut grant = Request::new(GrantAccessRequest {
        ...
        resource: "bucket:tenant-managed".to_string(),
        action: "bucket:read".to_string(),
    });
    add_bearer(&mut grant, &token);
    auth_client.grant_access(grant).await.unwrap();

    let token_before_rotate = get_token_for_scopes(
        &cluster.grpc_addrs[0],
        &created.client_id,
        &created.client_secret,
        vec!["bucket:read|bucket:tenant-managed".to_string()],
    )
    .await;
    ...
    let old_secret = try_get_token_for_scopes(
        &cluster.grpc_addrs[0],
        &created.client_id,
        &created.client_secret,
        vec!["bucket:read|bucket:tenant-managed".to_string()],
    ...
    let new_secret = get_token_for_scopes(
        &cluster.grpc_addrs[0],
        &rotated.client_id,
...
async fn tenant_cannot_manage_other_tenant_app_secret() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "tenant-b-app-manager".to_string(),
            vec!["app:rotate_secret|tenant:2".to_string()],
            2,
        )
        .unwrap();
    let mut rotate = Request::new(RotateApplicationCredentialSecretRequest {
...
async fn tenant_can_delegate_narrower_policy_capability() {
    ...
    let mut grant = Request::new(GrantAccessRequest {
        ...
        resource: "bucket:narrow/reports/".to_string(),
        action: "bucket:read".to_string(),
    });
    add_bearer(&mut grant, &token);
    auth_client.grant_access(grant).await.unwrap();
    ...
    let grants = auth_client
        ...
        .unwrap()
        .into_inner();
    assert!(grants.grants.iter().any(|grant| {
        grant.action == "bucket:read" && grant.resource == "bucket:narrow/reports/"
    }));
    ...
    let grantee_token = get_token_for_scopes(
        ...
        &grantee_client_id,
        &grantee_client_secret,
        vec!["bucket:read|bucket:narrow/reports/".to_string()],
    )
    .await;
    assert!(!grantee_token.is_empty());
}
...
async fn tenant_cannot_bind_host_alias_to_other_tenant_bucket() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "tenant-b-host-alias".to_string(),
            vec![format!("bucket:write|{bucket_name}")],
            2,
        )
        .unwrap();
    let mut object_client = ObjectServiceClient::connect(cluster.grpc_addrs[0].clone())
...
async fn tenant_diagnostics_are_tenant_scoped() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "tenant-b-diagnostics".to_string(),
    ...
    add_bearer(&mut denied, &tenant_b_token);
...
async fn tenant_repair_cannot_target_other_tenant() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token(
            "tenant-b-repair".to_string(),
            vec![format!("repair:run|{bucket_name}/search")],
            2,
        )
        .unwrap();
...
async fn tenant_audit_page_token_cannot_be_reused_by_other_tenant() {
    ...
    let tenant_b_token = cluster.states[0]
        .jwt_manager
        .mint_token("tenant-b-audit".to_string(), vec!["*|*".to_string()], 2)
        .unwrap();
    ...
    add_bearer(&mut reused, &tenant_b_token);
