...
pub fn validate_namespace_shape(namespace: &AuthzNamespaceSchema) -> Result<()> {
    ...
    if !namespace.schema_json.is_empty() {
        serde_json::from_str::<serde_json::Value>(&namespace.schema_json)
            .map_err(|error| contract_error(format!("authorization schema_json is invalid: {error}")))?;
    }
    Ok(())
}
    ...
    for selector in &member.allowed_subjects {
        ...
        match selector_kind {
            ...
            | AuthzSubjectSelectorKind::SameResourceId => {
                ...
                require_empty(&selector.subject_id, "allowed subject id", namespace, member)?;
            ...
            AuthzSubjectSelectorKind::Public => {
                require_empty(&selector.subject_kind, "public selector subject kind", namespace, member)?;
                require_empty(&selector.subject_id, "public selector subject id", namespace, member)?;
            }
...
fn validate_member_targets(
    ...
    if is_direct_relation(member) {
        for selector in &member.allowed_subjects {
                ...
                && selector.selector_kind == AuthzSubjectSelectorKind::Exact as i32
            {
                ...
                require_member(index, userset.namespace, userset.relation, "userset selector")?;
            }
        }
        return Ok(());
    }
    ...
    for rule in &member.rules {
        match rule.kind.as_str() {
            "inherit" => {
                require_member(index, &namespace.namespace, &rule.relation, "inherited member")?;
            }
            "computed" | "tuple_to_userset" => {
...
fn validate_tuple(
    ...
    let comparable_subject_id = if tuple.subject_kind == "userset" {
        ...
        require_member(index, userset.namespace, userset.relation, "userset subject")?;
        format!("{}/{}#{}", userset.namespace, userset.object_id, userset.relation)
    } else {
        tuple.subject_id.to_string()
    };
    let allowed = member.allowed_subjects.iter().any(|selector| {
        selector_matches(
            selector,
            tuple,
            comparable_subject_id.as_str(),
        )
    });
    if !allowed {
        return invalid(format!(
...
mod tests {
    ...
    fn accepts_typed_members_and_all_subject_selector_forms() {
        ...
        validate_tuple_batch(
            ...
            &[
                ...
                tuple("public_reader", "doc-1", PUBLIC_SUBJECT_KIND, PUBLIC_SUBJECT_ID),
    ...
    fn rejects_non_writable_members_and_selector_mismatches() {
        ...
        for (tuple, message) in [
            ...
            assert!(error.to_string().contains(message), "unexpected error: {error:#}");
        }
    }
    ...
    fn rejects_untyped_duplicate_and_structurally_mixed_members() {
        ...
        cases.push(namespace("document", vec![permission("viewer", Vec::new())]));
