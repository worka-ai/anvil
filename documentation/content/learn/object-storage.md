...
**What this page gives you:** a first-principles understanding of object storage. You will learn what buckets, keys, object bodies, metadata, versions, checksums, and range reads are before seeing how Anvil builds richer behaviour on top.
...
Create buckets around stable boundaries. Good bucket boundaries are things like product areas, tenants, datasets, artefact stores, or application domains. Avoid creating a bucket for every tiny folder; the key inside the bucket is usually the better place to express hierarchy.
...
With metadata, the application can ask for paid invoices for Acme without downloading every PDF. Anvil can also use metadata to scope full text search, vector search, watches, retention, and authorisation decisions.
...
Range reads matter for media playback, resumable downloads, partial archive inspection, and large model artefacts. They also need authorisation and version checks. A caller allowed to read one object version may not be allowed to read every related object.
...
- relationship authorisation for fine-grained visibility;
- watch streams so derived systems can catch up;
- PersonalDB witness state for local-first databases;
- source and model artefact manifests for reproducible data workflows.
