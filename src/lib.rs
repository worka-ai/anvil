...
impl AppState {
    pub async fn new(global_pool:Pool, regional_pool:Pool,region:String,jwt_secret:String)->Result<Self>{
...
pub async fn run(grpc_addr: SocketAddr) -> Result<()> {
    ...
    // --- State ---
    let state = AppState ::new(global_pool,regional_pool,region,env::var("JWT_SECRET").expect("JWT_SECRET must be set")).await?;
